...
class MenuDrawerController: UIViewController,UITableViewDelegate,UITableViewDataSource {
    ...
    var menuAry = ["", "My Profile","Camp Staff","Camp Listing","Camp Benefits", "Community","Invite a Friend","Blog", "FAQ’s","Contact Us" ,"Join our Team","Register your Camp","Terms of Service","Privacy Policy","Disclaimer","Sign Out"]
    var menuIcon = ["back (1)","back (1)", "back (1)","back (1)","back (1)", "back (1)", "back (1)","back (1)", "back (1)", "back (1)", "back (1)", "back (1)", "back (1)", "back (1)","back (1)", "sign-out"]
    ...
    func tableView(_ tableView: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        ...
        if auth == "" {
            ...
            if indexPath.row == 0 || indexPath.row == 1 || indexPath.row == 15 || indexPath.row == 11{
                return 0
            } else {
                return 70
            }
        ...
        if indexPath.row == 0 {
            ...
            if let wd = UIApplication.shared.delegate?.window {
                ...
                if(vc is UINavigationController){
                    ...
                    let nav = StoryboardChnage.mainStoryboard.instantiateViewController(withIdentifier: "NearMeCampsVC") as! NearMeCampsVC
            ...
            if let wd = UIApplication.shared.delegate?.window {
                ...
                if(vc is UINavigationController){
                    ...
                    let nav = StoryboardChnage.mainStoryboard.instantiateViewController(withIdentifier: "MemberBenefitsVC") as! MemberBenefitsVC
                    vc?.navigationController?.pushViewController(nav, animated: true)
                }
            }
            
        } else if indexPath.row  == 5 {
            delegate?.didSelected(index: 3)
        } else if indexPath.row  == 6 {
            delegate?.didSelected(index: 4)
        } else if indexPath.row  == 7 {
            delegate?.didSelected(index:5)
        } else if indexPath.row  == 8 {
            delegate?.didSelected(index: 6)
        } else if indexPath.row  == 9 {
            //            delegate?.didSelected(index: indexPath.row)
        } else if indexPath.row  == 10 {
            delegate?.didSelected(index: 8)
        }  else if indexPath.row  == 11 {
            delegate?.didSelected(index: 9)
        } else if indexPath.row  == 12 {
            delegate?.didSelected(index: 10)
        } else if indexPath.row  == 13 {
            delegate?.didSelected(index:11)
        } else if indexPath.row  == 14 {
            delegate?.didSelected(index: 12)
        } else if indexPath.row  == 15 {
            Proxy.shared.logout {
                KAppDelegate.isGradiantShownForHome=false
                self.delegate?.didSelected(index: 13)
            }
        }
    }
