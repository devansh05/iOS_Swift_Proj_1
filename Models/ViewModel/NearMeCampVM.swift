    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if NearMeCampVMObj.CampListModelAry.count != 0 {
            ...
            if UIDevice.current.userInterfaceIdiom == .pad{
                cell.imgVwBanner.sd_setIndicatorStyle(UIActivityIndicatorView.Style.whiteLarge)
            }else{
                cell.imgVwBanner.sd_setIndicatorStyle(UIActivityIndicatorView.Style.white)
            }
            cell.imgVwBanner.sd_addActivityIndicator()
            ...
            cell.imgVwBanner.sd_setImage(with: URL.init(string: "\(Apis.KCampTestUrl)\(CampListModelObj.arr_campBannerImg[0].stringValue)"), completed: { (image: UIImage?, error: Error?, cacheType: SDImageCacheType , imageURL: URL?) in
                ...
                cell.imgVwBanner.sd_removeActivityIndicator()
            })
