...
//            Proxy.shared.pushToNextVC(identifier: "CampfireVC", isAnimate: true, currentViewController: self)
    ...
    override func viewDidLoad() {
        ...
        if UIDevice.current.userInterfaceIdiom == .pad {
            ...
            if KAppDelegate.isCampfireTabSelected {
                //Devansh
                if UIDevice.current.userInterfaceIdiom == .pad {
    ...
    override func viewWillDisappear(_ animated: Bool) {
        KAppDelegate.isCampfireTabSelected=false
        KAppDelegate.isGradiantShownForCampfire = false
...
//        }
        ...
        view_GradiantRight.layer.sublayers=nil
        ...
        if tableView == tbl_LeftItems {
            if section == 0 {
                ...
                if self.arr_Advertisements.count > 0 {
                    if section >= 3 &&  self.numberOfAds < self.arr_Advertisements.count - 1 {
                        self.numberOfAds += 1
                    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if tableView == tbl_LeftItems {
            if indexPath.section == 0 {
                ...
                if indexPath.section >= 3  &&  self.numberOfAdsData < self.arr_Advertisements.count - 1 {
...
extension CampfireVC: UIScrollViewDelegate {
    
    func scrollViewDidScroll(_ scrollView: UIScrollView) {

        // initialize rect1 to the relevant subview
        if tour_VideoView.frame.intersects(CGRect(origin: scrollView.contentOffset, size: scrollView.frame.size)) {
            // the view is visible
            if self.player_Tour?.timeControlStatus != .playing {
                self.player_Tour?.play()
            }
            print("got video view @@@@@@@@")
        } else {
            self.player_Tour?.pause()
            print("out of the view @@@@@@@@")
        }
        
//        if scrollView.bounds.contains(tour_VideoView.frame) {
