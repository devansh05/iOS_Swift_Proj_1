...
protocol didselectAdvertisementDelegte{
    func didselectAdvertisement(adUrl:String, ad_device_Url: String)
            ...
            if GuestLandingModelAry.count != 0 {
                ...
                DispatchQueue.main.async {
                    if GuestLandingModelAryObj.type != "video" {
                        ...
                        DispatchQueue.main.async {
                            cell.imgVw.sd_setImage(with: URL.init(string: "\(Apis.KVideosThumbURL)\(GuestLandingModelAryObj.videoImageThumb)"),placeholderImage: nil, completed: nil)
                        }
...
//            }
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        if GuestLandingModelAryObj.type != "video" {
            ...
            delegate?.didselectAdvertisement(adUrl: url, ad_device_Url :url)
            return
        }
    ...
    {//varinder/12 //baltej
        ...
        let device_url = advertisement.value(forKey: "device_url") as? String ?? ""
        
        delegate?.didselectAdvertisement(adUrl: url, ad_device_Url :device_url)
    }
}
