...
class GuestLandingPageVC: UIViewController, SelectMenuOption ,SelectAviatorImage,TopHeaderViewDelegate,ManagePlyersDelegte {
    ...
    @IBOutlet weak var btnSignUp_Height_Constraints: NSLayoutConstraint!
    @IBOutlet weak var headerView: UIView!
    
    //varinder20
    var LblAttributedStr = NSAttributedString()
    //PLAYER TASK
    var player : AVPlayer?
    ...
    override func viewDidLoad() {
        ...
        if auth != "" {
            ...
            btnSignUp_Height_Constraints?.constant=0
            self.fetchProducts()
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        if UIDevice.current.userInterfaceIdiom == .pad {
            ...
            if slectedAviator != "" && !slectedAviator.contains("false") {
                ...
                view_HeaderView.imgVwAviator.image = UIImage(named : "defaultImg")
            }
    ...
    override func didRotate(from fromInterfaceOrientation: UIInterfaceOrientation) {
        ...
        if UIDevice.current.userInterfaceIdiom == .pad {
            if UIDevice.current.orientation == .portrait || UIDevice.current.orientation == .portraitUpsideDown {
                lblDescription.font = UIFont.boldSystemFont(ofSize: 19)
            }else{
                lblDescription.font = UIFont.boldSystemFont(ofSize: 21)
            }
            lblDescription.attributedText = LblAttributedStr
        }
    }
...
//                    if(!SRSubscriptionModel.shareKit().currentIsActive) {
    ...
    func videoPlayerPlay(Url: String,thumbURL:URL!,isAutoPlay:Bool) {
        if videoVw != nil {
            ...
            activityView.center = img_Thumb.center
            self.img_Thumb.addSubview(activityView)
            activityView.startAnimating()
            activityView.translatesAutoresizingMaskIntoConstraints = false
            
            NSLayoutConstraint.activate([
                activityView.widthAnchor.constraint(equalToConstant: 250),
                activityView.heightAnchor.constraint(equalToConstant: 100),
                activityView.centerXAnchor.constraint(equalTo: self.img_Thumb.centerXAnchor),
                activityView.centerYAnchor.constraint(equalTo: self.img_Thumb.centerYAnchor)
                ])
            
            if playerController != nil && isAutoPlay==true {
                
                self.img_Thumb.sd_setImage(with: URL(string: "https://wallpaperplay.com/walls/full/2/d/e/102437.jpg")!,placeholderImage: nil, completed: nil)
