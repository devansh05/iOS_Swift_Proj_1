        ...
        WebServiceProxy.shared.postDataForHome("\(Apis.KServerUrl)\(Apis.KHomeScreenApi)", params: param, showIndicator: true, completion: { (JSON) in
            ...
            if appResponse == 200 {
                ...
                if let dictResAry = JSON["results"] as? NSArray {
                    ...
                    //varinder24
                    let fullwidth_advts = JSON["advts_one"] as! [NSDictionary]
                    let fullwidth_small_height_advts = JSON["advt_two"] as! [NSDictionary]
                    let advt_three = JSON["advt_three"] as! [NSDictionary]
                    ...
                    if dictResAry.count > 0 {
                        ...
                        let CategoryModelObj3 = CategoryModel()
                        CategoryModelObj3.header_type = "Advertisement"
                        CategoryModelObj3.advertisment = advt_three
                        self.CategoryModelAry.insert(CategoryModelObj3, at: 8)
...
extension GuestLandingPageVC: UITableViewDelegate, UITableViewDataSource,UICollectionViewDelegate, UICollectionViewDataSource,UICollectionViewDelegateFlowLayout,didselectAdvertisementDelegte,didSelectUserCastScreen {
    ...
    func didselectAdvertisement(adUrl: String, ad_device_Url: String) {//varinder/12
        
        Proxy.shared.Adspush(identifier: ad_device_Url ,weburl : adUrl)
    }
